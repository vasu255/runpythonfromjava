...
public class JavaPythonInteropUnitTest {
	...
	public void givenPythonScript_whenPythonProcessInvoked_thenSuccess() throws Exception {
		...
		assertThat("Results should contain output of script: ", results,
				hasItem(containsString("Hello Baeldung Readers!!")));
	...
	@Test
	public void passing_values() throws Exception {
		int number1 = 100;
		int number2 = 200;
		ProcessBuilder processBuilder = new ProcessBuilder("python", resolvePythonScriptPath("pass_value.py"),
				"" + number1, "" + number2);
		processBuilder.redirectErrorStream(true);

		Process process = processBuilder.start();
		List<String> bfr = readProcessOutput(process.getInputStream());

		for (Object result : bfr) {
			System.out.println(result.toString());

		}
	}
	
	// Need to Install Jython 2.7.0 - Standalone Jar.
	@Test
	public void call_function() {
		PythonInterpreter interpreter = new PythonInterpreter();
	    interpreter.execfile(resolvePythonScriptPath("add.py"));
	    PyObject str = interpreter.eval("repr(add(4,5))");
